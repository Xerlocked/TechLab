...
void AEditorPlayer::Input()
{
    ...
    bool bIsUpNow = (UpKey & 0x8000) != 0;
    bool bIsSpaceNow = (SpaceKey & 0x8000) != 0;

    // 눌렀을 때 처리
    if (bIsUpNow && !bWasUpPressed)
    {
        GEngineLoop.bIsWalking = true;
    }

    if (bIsSpaceNow && !bWasSpacePressed)
    {
        GEngineLoop.bIsJumping = true;
    }

    // 땠을 때 처리
    if (!bIsUpNow && bWasUpPressed)
    {
        GEngineLoop.bIsWalking = false;
    }

    if (!bIsSpaceNow && bWasSpacePressed)
    {
        GEngineLoop.bIsJumping = false;
    }

    // 현재 상태 저장
    bWasUpPressed = bIsUpNow;
    bWasSpacePressed = bIsSpaceNow;
